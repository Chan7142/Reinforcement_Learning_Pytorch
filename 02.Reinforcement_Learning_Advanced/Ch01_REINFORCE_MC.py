...
def update(logPi_list, reward_list):
    ...
    loss = -(logPi_traj*gamma_traj*Rt_traj).sum()
    optimizer.zero_grad()
    loss.backward()
    optimizer.step()
...
plt.title('REINFORCE_MC training')
plt.ylabel('Duration')
plt.xlabel('Episode')
plt.plot(episode_durations)
plt.savefig('REINFORCE_MC_training.png', dpi = 300, bbox_inches = 'tight')
plt.show()
